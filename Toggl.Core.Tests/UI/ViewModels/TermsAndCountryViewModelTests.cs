...
namespace Toggl.Core.Tests.UI.ViewModels
{
    public sealed class TermsAndCountryViewModelTests
    {
        ...
        {
            [Fact, LogIfTooSlow]
            public async Task TracksTaCPrivacyPolicyOpened()
            {
                Api.Location.Get().ReturnsThrowingTaskOf(new Exception());

                ViewModel.ViewPrivacyPolicy.Execute();

                AnalyticsService.TaCPrivacyPolicyOpened.Received().Track();
            }

            [Fact, LogIfTooSlow]
            public async Task TracksTaCTermsOfServiceOpened()
            {
                Api.Location.Get().ReturnsThrowingTaskOf(new Exception());

                ViewModel.ViewTermsOfService.Execute();

                AnalyticsService.TaCTermsOfServiceOpened.Received().Track();
            }

            [Fact, LogIfTooSlow]
            public async Task TracksTaCAgreeButtonTapped()
            {
                Api.Location.Get().ReturnsThrowingTaskOf(new Exception());

                ViewModel.Accept.Execute();

                AnalyticsService.TaCAgreeButtonTapped.Received().Track();
            }
        }
        ...
        public sealed class ThePickCountryMethod : TermsAndCountryViewModelTest
        {
            ...
            [Fact, LogIfTooSlow]
            public async Task TracksTaCSelectedCountryWhenCountryNotSelected()
            {
                Api.Location.Get().ReturnsThrowingTaskOf(new Exception());
                NavigationService
                    .Navigate<SelectCountryViewModel, long?, long?>(Arg.Any<long?>(), ViewModel.View)
                ...
                AnalyticsService.TaCCountryNotSelected.Received().Track();
            }
            ...
            public async Task TracksSelectedCountryCode()
            {
                ...
                AnalyticsService.TaCSelectedCountry.Received().Track(selectedCountry.CountryCode);
            }
        }
    }
}
